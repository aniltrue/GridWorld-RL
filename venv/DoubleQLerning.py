...
class DoubleQAgent(Agent):
    def __init__(self, env: Environment, discountRate=0.995, epsilon=1.0, epsilonDecay=0.995, epsilonMin=0.1,
                 alpha=0.2):
        '''
        ...
        self.Q1 = np.zeros((self.stateSize, self.actionSize))
        self.Q2 = np.zeros((self.stateSize, self.actionSize))
    ...
    def train(self, TDLimit=0.01, printDetails=True, **kwargs):
        ...
        for episode in range(maxEpisode):
            ...
            while not done:
                ...
                if rnd.random() < 0.5:
                    nextAction = np.argmax(self.Q1[nextState])
                    q = reward + self.discountRate * self.Q2[nextState][nextAction] if not done else reward
                    TD = q - self.Q1[currentState, action]

                    self.Q1[currentState][action] += self.alpha * TD
                else:
                    nextAction = np.argmax(self.Q2[nextState])
                    q = reward + self.discountRate * self.Q1[nextState][nextAction] if not done else reward
                    TD = q - self.Q2[currentState, action]

                    self.Q2[currentState][action] += self.alpha * TD
